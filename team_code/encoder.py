...
class CLIPVisionTower(nn.Module):
    ...
    def load_model(self, path: str = None):
        if path is None:
            print(f"Load vision model from `vision_tower_name`: {self.vision_tower_name}")
            self.image_processor = CLIPImageProcessor.from_pretrained(self.vision_tower_name)
            self.vision_tower = CLIPVisionModel.from_pretrained(self.vision_tower_name)
        else:
            print(f"Load vision model from `path`: {path}")
            self.image_processor = CLIPImageProcessor.from_pretrained(path, local_files_only=True)
            self.vision_tower = CLIPVisionModel.from_pretrained(path, local_files_only=True)
