...
def create_photo_grid(arr, rows=None, cols=None):
    ...
    if isinstance(arr, list):
        if isinstance(arr[0], Image.Image):
            ...
            raise ValueError(
                "Invalid input type. Expected list of Images or numpy arrays."
            )
    ...
    if rows * cols < t:
        raise ValueError(
            f"Not enough grid cells ({rows}x{cols}) to hold all images ({t})."
        )
    ...
    for i in range(t):
        row_idx = i // cols
        col_idx = i % cols
        grid[row_idx * h : (row_idx + 1) * h, col_idx * w : (col_idx + 1) * w, :] = arr[
            i
        ]
...
def tokenizer_multimodal_token(
    prompt, tokenizer, multimodal_token=DEFAULT_IMAGE_TOKEN, return_tensors=None
):
...
def waveform2melspec(
    ...
    if mel.shape[0] > target_length:
        ...
        mel_fusion = torch.stack(
            [mel_chunk_front, mel_chunk_middle, mel_chunk_back], dim=0
        )
        ...
        mel_fusion = torch.stack([mel, mel, mel], dim=0)
    mel_fusion = mel_fusion.transpose(
        1, 2
    )  # [3, target_length, mel_bins] -> [3, mel_bins, target_length]
...
def process_audio(
    ...
    if sample_rate != origin_sr:
        ...
        audio_data = torchaudio.functional.resample(
            audio_data, orig_freq=origin_sr, new_freq=sample_rate
        )
    waveform_melspec = waveform2melspec(
        audio_data, sample_rate, num_mel_bins, target_length, audio_mean, audio_std
    )
