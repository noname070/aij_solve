    ...
    **kwargs,
    ...
    if not os.path.exists(model_path):
        ...
        raise FileExistsError(
            f"Model's checkpoint was not found by path: {vision_tower_model_path}"
        )
...
def process_data_sample(
    ...
    if modality_path is None:
        raise ValueError(
            f"Modality type: `{modality_type}` was not found in input data sample!"
        )
    ...
    **kwargs,
    ...
    if modality_type == "text":
        ...
        if modality_features is None:
            raise ValueError(
                f"Modality type: {modality_type} was not found in input data!"
            )
    ...
    input_ids = (
        tokenizer_multimodal_token(prompt, tokenizer, modal_token, return_tensors="pt")
        .unsqueeze(0)
        .to(model.device)
    )
    ...
    **kwargs,
    ...
    for i, opt in enumerate(options):
        ...
        res.append(
            (
                float(loss.item()),
                float((loss / (labels != -100).sum()).item()),
                bool(max_equal),
            )
        )
